...
### Choosing a model for each teammate

When spawning a subagent with the Task tool, choose the `model` based on the task complexity:

- *"opus"* — Deep reasoning, complex architecture, nuanced analysis, creative writing
- *"sonnet"* — Coding, research, most general tasks (good default)
- *"haiku"* — Simple lookups, formatting, data extraction, quick checks

Match the model to the job. A researcher analyzing scientific papers should use opus or sonnet. An agent just fetching a URL and summarizing it can use haiku. When in doubt, use sonnet.
